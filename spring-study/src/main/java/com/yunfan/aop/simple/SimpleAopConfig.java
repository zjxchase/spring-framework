...
// *
// * 1、导入模块： Spring AOP (spring-aspects)
// * 2、定义一个业务逻辑类（MathCalculator）:在业务逻辑运行的时候将日志进行打印（方法之前、方法运行结束、方法出现异常）
// * 3、定义一个日志切面类（LogAspects），切面类里面的方法需要动态的感知MathCalculator.div运行到哪里然后执行
// *      通知方法：
// *          前置通知(@Before)：logStart：在目标方法（div）运行之前运作
// *          后置通知(@After)：logEnd:在目标方法（div）运行结束之后运行(无论方法是正常结束还是异常结束都调用)
// *          返回通知(@AfterReturning)：logReturn:在目标方法（div）正常返回之后运行
// *          异常通知(@AtferThrowing)：logException:在目标方法（div）出现异常以后运行
...
// *      在Spring中很多的@EnableXXX;表示开启某一项功能
// *
// * 三步
// *      1）、将业务逻辑组件和切面类都放到容器中：告诉Spring那个是切面类（@Aspect）
// *      2）、在切面类上的每一个方法通知上标注通知注解，告诉Spring何时何地运行（切入点表达式）
...
// *                          implements SmartInstantiationAwareBeanPostProcessor(bean的后置处理器), BeanFactoryAware（可以把bean工厂传进来）
// *                      关注后置处理器（在bean初始化完成前后做的事情），自动装配BeanFactory
// *
// *
// *  查找和后置处理器、自动装配相关的功能：
// *
// * 第一级父类：AbstractAutoProxyCreator：
...
// *  AbstractAutoProxyCreator.有后置处理器的逻辑 如：方法名为postProcessXXX 的方法, 只需要关注有逻辑的，直接返回值的不需要关注
// *
// * 第二级父类：AbstractAdvisorAutoProxyCreator：
// * org.springframework.aop.framework.autoproxy.AbstractAdvisorAutoProxyCreator
// *  重写了父类（AbstractAutoProxyCreator） 的 setBeanFactory()
...
// *
// * 第三级父类：AspectJAwareAdvisorAutoProxyCreator
// * org.springframework.aop.aspectj.autoproxy.AspectJAwareAdvisorAutoProxyCreator
// *
// * 本类：AnnotationAwareAspectJAutoProxyCreator
...
// *                      1）、创建Bean的实例
// *                      2）、pupulateBean：给bean的各种属性赋值
// *                      3）、initializBean:初始化bean:
// *                          1)、invokeAware（）：处理Aware接口的方法回调
...
// *                  只要创建好的Bean都会被缓存起来
// *              2）、createBean();创建bean；AnnotationAwareAspectJAutoProxyCreator会在任何bean创建之前先尝试返回bean的（代理）实例
...
// *          4、
// *      4）、给容器中返回当前最近使用cglib增强了的代理对象；
// *      5）、以后容器中获取到的就是这个组件的代理对象，执行目标方法的时候，代理对象就会运行通知方法的流程：
// *      6）、
// *  3）、目标方法的执行
...
// *          4）、
// *      3）、如果没有拦截器链，直接执行目标方法
// *          拦截器链（每一个通知方法又被包装为方法拦截器，使用MethodInterceptor机制）
// *      4）、如果有拦截器链，把需要执行的目标对象，目标方法，拦截器链等信息传入创建一个CglibMethodInvocation对象，
// *          并调用var retVal = new CglibMethodInvocation(proxy, target, method, args, targetClass, chain, methodProxy).proceed(); 即 proceed（） 方法
// *      5）、拦截器链的触发过程；
// *          1）、如果没有拦截器执行目标方法，或者拦截器的索引和拦截器数组 - 1 大小一样（执行到了最后一个拦截器）执行目标方法
